...
const AddTaskModal = async () => {
  ...
  const addTaskPriorityLevel = document.getElementById(
    "add-task-modal-priorityLevel"
  );

  if (
    addTaskTitle.value.trim() == "" &&
    addTaskDescription.value.trim() == ""
  ) {
    document.getElementById(
      "add-task-modal-title-message"
    ).innerHTML = `<div role="alert" class="alert alert-warning h-10 ">
  ...
  const data = await AddTask(
    ...
    addTaskPriorityLevel.value,
    addTaskAssignee.value,
